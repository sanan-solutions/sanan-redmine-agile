...
//     if (!card.matches('.issue-card')) return;

//     var map = window.SANAN_TRACKER_COLORS || {};
//     var mode = (window.SANAN_TRACKER_COLOR_MODE || 'body').toLowerCase();
//     if (!map || Object.keys(map).length === 0) return;

//     var tracker = extractTrackerName(card);
//     if (!tracker) return;

//     var color = map[tracker];
//     if (!color) return;

//     // 🔥 IDPOTENT CHECK:
//     // Nếu màu + chế độ không thay đổi → bỏ qua
//     var prevKey = card.dataset._saColorKey;
//     var newKey = tracker + '::' + color + '::' + mode;

//     if (prevKey === newKey) return; // không làm lại

//     // lưu key mới
//     card.dataset._saColorKey = newKey;
